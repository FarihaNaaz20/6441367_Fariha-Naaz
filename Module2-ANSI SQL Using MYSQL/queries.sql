...
GROUP BY e.event_id;
-- 11. Daily New User Count: Number of users registered each day in the last 7 days
SELECT 
    registration_date, 
...
-- 12. Event with Maximum Sessions: Event(s) with highest number of sessions
SELECT 
    e.title, 
...
-- 13. Average Rating per City: Average feedback rating of events in each city
SELECT 
    e.city, 
    ROUND(AVG(f.rating), 2) AS avg_rating
FROM Events e
...
-- 19. Completed Events with Feedback Summary: Total registrations and avg feedback for completed events
SELECT 
    e.title,
...
-- 20. User Engagement Index: For each user, count of events attended and feedbacks submitted
SELECT 
    u.full_name,
...
-- 21. Top Speakers by Session Count
...
-- 22. Events Without Registrations
...
-- 23. Average Feedback per Event Status

SELECT e.status, ROUND(AVG(f.rating), 2) AS avg_rating
FROM Events e
LEFT JOIN Feedback f ON e.event_id = f.event_id
GROUP BY e.status;

-- 24. User Registration Frequency

SELECT u.full_name, COUNT(r.registration_id) AS registrations_count
FROM Users u
LEFT JOIN Registrations r ON u.user_id = r.user_id
GROUP BY u.user_id
ORDER BY registrations_count DESC;

-- 25. Latest Feedback Comments per Event

SELECT e.title, f.comments, f.feedback_date
FROM Feedback f
JOIN Events e ON f.event_id = e.event_id
WHERE (f.event_id, f.feedback_date) IN (
    SELECT event_id, MAX(feedback_date)
    FROM Feedback
    GROUP BY event_id
)
ORDER BY e.title;
